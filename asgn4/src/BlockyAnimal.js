...
var FSHADER_SOURCE = `
  ...
  vec3 applyPhongLight(vec3 lightPos, vec3 lightColor, vec3 normal, vec3 viewDir) {
    vec3 lightDir = normalize(lightPos - v_WorldPos);
    float diffuse = max(dot(normal, lightDir), 0.0);
    vec3 reflectDir = reflect(-lightDir, normal);
    float specular = pow(max(dot(viewDir, reflectDir), 0.0), 24.0);
    return lightColor * (diffuse * u_FragColor.rgb + specular * vec3(0.55));
  }
  ...
  void main() {
    ...
    vec3 color = u_FragColor.rgb * 0.22;

    if (u_PointLightOn) {
      color += applyPhongLight(u_LightPos, u_LightColor, normal, viewDir);
    }

    if (u_SpotLightOn) {
      vec3 fromSpot = normalize(v_WorldPos - u_SpotPos);
      float theta = dot(normalize(u_SpotDir), fromSpot);
      float spotFactor = smoothstep(0.82, 0.94, theta);
      color += applyPhongLight(u_SpotPos, u_SpotColor * spotFactor, normal, viewDir);
    }

    gl_FragColor = vec4(min(color, vec3(1.0)), u_FragColor.a);
  }`;

const EMBEDDED_OBJ_DATA = `# Low poly orb
o LowPolyOrb
v 0.0000 0.8507 0.0000
v 0.4472 0.7236 0.2764
v -0.1708 0.7236 0.4961
v -0.5528 0.7236 0.0000
v -0.1708 0.7236 -0.4961
v 0.4472 0.7236 -0.2764
v 0.7236 0.2764 0.4472
v 0.0000 0.2764 0.7236
v -0.7236 0.2764 0.4472
v -0.7236 0.2764 -0.4472
v 0.0000 0.2764 -0.7236
v 0.7236 0.2764 -0.4472
v 0.5528 -0.2764 0.0000
v 0.1708 -0.2764 0.4961
v -0.4472 -0.2764 0.2764
v -0.4472 -0.2764 -0.2764
v 0.1708 -0.2764 -0.4961
v 0.7236 -0.2764 -0.0000
v 0.4472 -0.7236 0.2764
v -0.1708 -0.7236 0.4961
v -0.5528 -0.7236 0.0000
v -0.1708 -0.7236 -0.4961
...
let g_pointLightMotionOn = true;
let g_spotLightOn = true;
let g_lightColor = [1.0, 0.92, 0.82];
let g_lightSlider = { x: 0.0, y: 0.95, z: 0.0 };
let g_lightPos = [0.0, 0.95, 0.0];
let g_spotPos = [-1.15, 1.5, 1.2];
let g_spotDir = [0.55, -0.72, -0.42];
let g_spotColor = [0.45, 0.62, 0.95];
let g_cameraEye = [0.0, 0.2, 3.2];

let g_loadedModel = null;
let g_objLoaded = false;
let g_objStatus = 'OBJ: loading...';
...
function connectVariablesToGLSL() {
  ...
  if (a_Position < 0 || a_Normal < 0 || !u_FragColor || !u_ModelMatrix || !u_GlobalRotateMatrix ||
      !u_ViewMatrix || !u_ProjMatrix || !u_NormalMatrix || !u_LightPos || !u_LightColor ||
      !u_SpotPos || !u_SpotDir || !u_SpotColor || !u_CameraPos || !u_LightingOn ||
      !u_NormalVisualization || !u_PointLightOn || !u_SpotLightOn) {
  ...
  gl.uniformMatrix4fv(u_GlobalRotateMatrix, false, identityMatrix.elements);
  gl.uniformMatrix4fv(u_NormalMatrix, false, identityMatrix.elements);
}

function sendShapeUniforms(modelMatrix) {
  gl.uniformMatrix4fv(u_ModelMatrix, false, modelMatrix.elements);

  const modelWorldMatrix = new Matrix4(g_globalRotateMatrix);
  modelWorldMatrix.multiply(modelMatrix);

  const normalMatrix = new Matrix4();
  normalMatrix.setInverseOf(modelWorldMatrix);
...
function updateLightButtonLabels() {
  setButtonLabel('lightingToggleButton', g_lightingOn ? 'Lighting ON' : 'Lighting OFF');
  setButtonLabel('pointLightToggleButton', g_pointLightOn ? 'Point Light ON' : 'Point Light OFF');
  setButtonLabel('pointLightMotionButton', g_pointLightMotionOn ? 'Point Motion ON' : 'Point Motion OFF');
  setButtonLabel('spotLightToggleButton', g_spotLightOn ? 'Spot Light ON' : 'Spot Light OFF');
  setButtonLabel('normalToggleButton', g_normalVisualization ? 'Show Normals ON' : 'Show Normals OFF');
}
...
function syncControls() {
  const values = {
    ...
    lightXSlide: Math.round(g_lightSlider.x * 100),
    lightYSlide: Math.round(g_lightSlider.y * 100),
    lightZSlide: Math.round(g_lightSlider.z * 100),
    lightRSlide: Math.round(g_lightColor[0] * 100),
    lightGSlide: Math.round(g_lightColor[1] * 100),
    lightBSlide: Math.round(g_lightColor[2] * 100)
  };
...
function addActionsForHtmlUI() {
  ...
  addSliderControl('lightXSlide', function(value) { g_lightSlider.x = value / 100; });
  addSliderControl('lightYSlide', function(value) { g_lightSlider.y = value / 100; });
  addSliderControl('lightZSlide', function(value) { g_lightSlider.z = value / 100; });
  addSliderControl('lightRSlide', function(value) { g_lightColor[0] = value / 100; });
  addSliderControl('lightGSlide', function(value) { g_lightColor[1] = value / 100; });
  addSliderControl('lightBSlide', function(value) { g_lightColor[2] = value / 100; });

  const animation1Button = document.getElementById('animation1Button');
  if (animation1Button) {
    animation1Button.onclick = function() { setAnimationMode(1); };
  }

  const animation2Button = document.getElementById('animation2Button');
  if (animation2Button) {
    animation2Button.onclick = function() { setAnimationMode(2); };
  }

  const animation3Button = document.getElementById('animation3Button');
  if (animation3Button) {
    animation3Button.onclick = function() { setAnimationMode(3); };
  }

  const animation4Button = document.getElementById('animation4Button');
  if (animation4Button) {
    animation4Button.onclick = function() { setAnimationMode(4); };
  }

  const animationOffButton = document.getElementById('animationOffButton');
  if (animationOffButton) {
    animationOffButton.onclick = function() { setAnimationMode(0); };
  }
  ...
  const lightingToggleButton = document.getElementById('lightingToggleButton');
  if (lightingToggleButton) {
    lightingToggleButton.onclick = function() {
  ...
  const pointLightToggleButton = document.getElementById('pointLightToggleButton');
  if (pointLightToggleButton) {
    pointLightToggleButton.onclick = function() {
  ...
  const pointLightMotionButton = document.getElementById('pointLightMotionButton');
  if (pointLightMotionButton) {
    pointLightMotionButton.onclick = function() {
      g_pointLightMotionOn = !g_pointLightMotionOn;
      updateLightButtonLabels();
      renderScene();
    };
  }

  const spotLightToggleButton = document.getElementById('spotLightToggleButton');
  if (spotLightToggleButton) {
    spotLightToggleButton.onclick = function() {
  ...
  const normalToggleButton = document.getElementById('normalToggleButton');
  if (normalToggleButton) {
    normalToggleButton.onclick = function() {
...
async function loadOBJSceneModel() {
  g_loadedModel = new OBJModel();
  g_loadedModel.color = [0.72, 0.77, 0.84, 1.0];

  try {
    await g_loadedModel.load('lowpoly_orb.obj', EMBEDDED_OBJ_DATA);
    g_objLoaded = true;
    g_objStatus = 'OBJ: loaded lowpoly_orb.obj';
  } catch (error) {
    g_objLoaded = false;
    g_objStatus = 'OBJ: failed to load';
  }
...
function updateLightPosition() {
  ...
  const orbitX = g_pointLightMotionOn ? Math.cos(g_seconds * 0.9) * orbitRadius : 0.0;
  const orbitZ = g_pointLightMotionOn ? Math.sin(g_seconds * 0.9) * orbitRadius : 0.0;
  g_lightPos = [
    g_lightSlider.x + orbitX,
    g_lightSlider.y,
    g_lightSlider.z + orbitZ
  ];
}
...
function renderSphereShape(matrix, color, latBands, longBands) {
  const sphere = new Sphere();
  sphere.color = color;
  sphere.matrix = matrix;
  if (latBands) {
    sphere.latBands = latBands;
  }
  if (longBands) {
    sphere.longBands = longBands;
  }
  sphere.render();
}
...
function renderTail(tailBase) {
  ...
  renderTailStrand(root, 0.0, 0.0, 0, [0.072, 0.068, 0.062, 0.056, 0.049, 0.041, 0.033], [0.11, 0.11, 0.105, 0.1, 0.095, 0.088, 0.08]);
  renderTailStrand(root, -0.004, -0.022, 10, [0.05, 0.047, 0.043, 0.038, 0.033, 0.028, 0.022], [0.1, 0.1, 0.096, 0.092, 0.088, 0.082, 0.075]);
  renderTailStrand(root, -0.004, 0.022, -10, [0.05, 0.047, 0.043, 0.038, 0.033, 0.028, 0.022], [0.1, 0.1, 0.096, 0.092, 0.088, 0.082, 0.075]);
  renderTailStrand(root, 0.002, -0.04, 18, [0.034, 0.032, 0.029, 0.026, 0.022, 0.019, 0.016], [0.09, 0.09, 0.087, 0.083, 0.079, 0.074, 0.068]);
  renderTailStrand(root, 0.002, 0.04, -18, [0.034, 0.032, 0.029, 0.026, 0.022, 0.019, 0.016], [0.09, 0.09, 0.087, 0.083, 0.079, 0.074, 0.068]);
}
...
function renderSceneObjects() {
  ...
  sphereMatrix.translate(0.95, -0.22, -0.05);
  sphereMatrix.scale(0.26, 0.26, 0.26);
  renderSphereShape(sphereMatrix, [0.84, 0.9, 0.98, 1.0], 18, 24);

  const sphereMatrix2 = new Matrix4();
  sphereMatrix2.translate(0.7, -0.08, 0.72);
  sphereMatrix2.scale(0.14, 0.14, 0.14);
  renderSphereShape(sphereMatrix2, [0.94, 0.8, 0.63, 1.0], 14, 18);

  if (g_loadedModel && g_loadedModel.loaded) {
    const modelMatrix = new Matrix4();
    modelMatrix.translate(1.15, -0.38, -0.78);
    modelMatrix.rotate(g_seconds * 24.0, 0, 1, 0);
    modelMatrix.scale(0.42, 0.42, 0.42);
    g_loadedModel.matrix = modelMatrix;
    g_loadedModel.render();
  }
}

function renderPointLightMarker() {
  const marker = new Matrix4();
  marker.translate(g_lightPos[0], g_lightPos[1], g_lightPos[2]);
  marker.scale(0.08, 0.08, 0.08);
  renderCube(marker, [g_lightColor[0], g_lightColor[1], g_lightColor[2], 1.0]);
}
...
function renderScene() {
  ...
  g_globalRotateMatrix.rotate(g_globalXAngle, 1, 0, 0);
  g_globalRotateMatrix.rotate(g_globalAngle, 0, 1, 0);
  ...
  renderHorse();
  renderSceneObjects();
  renderPointLightMarker();

  const duration = performance.now() - startTime;
  sendTextToHTML('ms: ' + duration.toFixed(1) + ' fps: ' + g_fps.toFixed(1), 'numdot');
  sendTextToHTML(g_objStatus, 'objStatus');
}
